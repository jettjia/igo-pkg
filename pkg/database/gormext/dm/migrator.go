...
func (m Migrator) FullDataTypeOfAlert(field *schema.Field) clause.Expr {
	expr := m.Migrator.FullDataTypeOf(field)
...
func (m Migrator) AlterColumn(value interface{}, field string) error {
	return m.RunWithValue(value, func(stmt *gorm.Statement) error {
		if field := stmt.Schema.LookUpField(field); field != nil {
			// dm 在int类型上设置unique。alter会报错
			if field.Unique && field.DataType != "string" {
				return nil
			}
			// float类型不支持长度
			if field.GORMDataType == "float" {

				fieldRes, err := MysqlGormType2Dm(field)
				if err != nil {
					return err
				}
				field.DataType = fieldRes.DataType
			}
			return m.DB.Exec(
				"ALTER TABLE ? MODIFY ? ?",
				clause.Table{Name: stmt.Table},
				clause.Column{Name: field.DBName},
				m.FullDataTypeOfAlert(field),
			).Error
		}
