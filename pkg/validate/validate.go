...
func init() {
	...
	// registration custom error
	if err := validate.RegisterValidation("checkSpecialChar", checkSpecialChar); err != nil {
		panic(any(err))
	}
	// registration custom error
	if err := validate.RegisterValidation("trim_spaces", trimSpaces); err != nil {
		panic(any(err))
	}
}
...
// checkSpecialChar verify special characters
...
// trimSpaces trims spaces from both ends of a string before validation.
func trimSpaces(f validator.FieldLevel) bool {
	value := f.Field().String()
	trimmedValue := strings.TrimSpace(value)
	f.Field().SetString(trimmedValue)
	return trimmedValue != ""
}
