...
func (b *graphBuilder) extractEntities(ctx context.Context, chunk *types.Chunk) ([]*types.Entity, error) {
	...
	messages := []chat.Message{
		{
			Role:    "system",
			Content: b.config.Ai.ExtractEntitiesPrompt,
		},
		{
			Role:    "user",
...
func (b *graphBuilder) extractRelationships(ctx context.Context,
	...
	messages := []chat.Message{
		{
			Role:    "system",
			Content: b.config.Ai.ExtractRelationshipsPrompt,
		},
		{
			Role:    "user",
	...
	// 添加日志查看LLM返回的原始内容
	log.Debugf("LLM response content: %s", resp.Content)
	if err := common.ParseLLMJsonResponse(resp.Content, &extractedRelationships); err != nil {
		log.WithError(err).Errorf("Failed to parse relationship extraction response, content: %s", resp.Content)
...
func (b *graphBuilder) BuildGraph(ctx context.Context, chunks []*types.Chunk) error {
	...
	for i, batchChunks := range common.ChunkSlice(chunks, relationChunkSize) {
