...
func (e *batchEmbedder) BatchEmbedWithPool(ctx context.Context, model Embedder, texts []string) ([][]float32, error) {
	...
	processChunk := func(texts []*textEmbedding) func() {
		return func() {
			...
			// Embed text
			embedding, err := model.BatchEmbed(ctx, common.MapSlice(texts, func(text *textEmbedding) string {
				return text.text
			}))
			if err != nil {
				mu.Lock()
	...
	for _, texts := range common.ChunkSlice(textEmbeddings, batchSize) {
		wg.Add(1)
	...
	results := common.MapSlice(textEmbeddings, func(text *textEmbedding) []float32 {
		return text.results
	})
	return results, nil
}
