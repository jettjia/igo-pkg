...
func init() {
	flag.StringVar(&confPath, "conf", "", "config path, doc: -conf /config.yaml")
}

// ParseYaml Parse YAML Format File
func ParseYaml(confPtr interface{}) error {
	return parse(_Yaml, confPtr)
}

// ParseToml Parse TOML Format File
func ParseToml(confPtr interface{}) error {
	return parse(_Toml, confPtr)
}

// ParseJson Parse JSON Format File
